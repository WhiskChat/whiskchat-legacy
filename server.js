...
app.post('/github', function(req, res) {
	...
	sockets.forEach(function(sock) {
            sock.emit('chat', {room: 'main', message: '<center><strong><i class="icon-hdd"></i> ' + payload.commits[0].author.username + ': Commit ' + payload.after.substr(0, 6) + ' @ ' + payload.ref.split('/').pop() + ' (' + decodeURIComponent(payload.commits[0].message).replace(/\+/g, " ") + ')</strong></center>', user: 'GitHub', timestamp: Date.now()});
	});
	res.writeHead(200);
	res.end();
    });
...
io.sockets.on('connection', function(socket) {
	...
	if (!socket.authed) {
		...
		return chatemit(socket, '<span style="text-shadow: 3px 3px 0 rgba(64,64,64,0.4),-3px -3px 0px rgba(64,64,64,0.2); font-size: 3em; color: #1CFFFB;">' + stripHTML(chat.message.substr(3, chat.message.length)) + '</span>', chat.room);
            ...
            if (chat.message.substr(0,8) == "/kickall") { // By DiamondCardz
                if (socket.rank !== 'admin') {
                    socket.emit("message", {type: "alert-error", message: "You do not have permissions to kick everyone!"});
		    return; 
		}
		sockets.forEach(function(sock) {
